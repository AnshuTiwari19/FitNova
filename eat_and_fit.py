...
# Center logo and title using columns
...
with st.sidebar:
    ...
    sex_input = st.radio("**What's your sex?**", ('Male', 'Female'), key='sex_input_value', 
                         index=st.session_state.page1['sex'], on_change=submit_sex)

    height_input = st.number_input("**What's your height (in centimeters)?**", key='height_input_value', 
                                   min_value=130.0, max_value=220.0, step=0.1, 
                                   value=st.session_state.page1['height'], on_change=submit_height)

    weight_input = st.number_input("**What's your weight (in kilograms)?**", key='weight_input_value', 
                                   min_value=30.0, max_value=150.0, step=0.1, 
                                   value=st.session_state.page1['weight'], on_change=submit_weight)

    stage_input = st.selectbox("**Are you new to weight loss?**", 
                               ("Yes, I'm a beginner", "No, I'm an intermediate"), 
                               key='stage_input_value', index=st.session_state.page1['stage'], 
                               on_change=submit_stage)

    # Align buttons in sidebar
...
if not st.session_state.page1['is_first_load']:
    # Perform Fuzzy Logic to determine body state
    ...
    if body == 0:
        ...
        # Layout for displaying diet plans
        ...
        with engine.connect() as conn:
            ...
            lc_result = conn.execute(
                text("SELECT * FROM LowCarb WHERE Calories = :calories"),
            ...
            def get_dish_by_id(dish_id):
                return Dish(*conn.execute(
                    text("SELECT * FROM Dish WHERE Id = :id"),
                    {'id': dish_id}
                ).fetchone())

            low_carb_breakfast_1 = get_dish_by_id(low_carb_breakfast_detail.id1)
            low_carb_breakfast_2 = get_dish_by_id(low_carb_breakfast_detail.id2)
            low_carb_lunch_1 = get_dish_by_id(low_carb_lunch_detail.id1)
            low_carb_lunch_2 = get_dish_by_id(low_carb_lunch_detail.id2)
            low_carb_dinner_1 = get_dish_by_id(low_carb_dinner_detail.id1)
            low_carb_dinner_2 = get_dish_by_id(low_carb_dinner_detail.id2)

            # Display UI for low carb diet
            with low_carb_1:
                st.markdown(
                    f"""
            ...
            lc_result = conn.execute(
                text("SELECT * FROM ModerateCarb WHERE Calories = :calories"),
            ...
            moderate_carb_breakfast_1 = get_dish_by_id(moderate_carb_breakfast_detail.id1)
            moderate_carb_breakfast_2 = get_dish_by_id(moderate_carb_breakfast_detail.id2)
            moderate_carb_lunch_1 = get_dish_by_id(moderate_carb_lunch_detail.id1)
            moderate_carb_lunch_2 = get_dish_by_id(moderate_carb_lunch_detail.id2)
            moderate_carb_dinner_1 = get_dish_by_id(moderate_carb_dinner_detail.id1)
            moderate_carb_dinner_2 = get_dish_by_id(moderate_carb_dinner_detail.id2)
            ...
            high_carb_breakfast_1 = get_dish_by_id(high_carb_breakfast_detail.id1)
            high_carb_breakfast_2 = get_dish_by_id(high_carb_breakfast_detail.id2)
            high_carb_lunch_1 = get_dish_by_id(high_carb_lunch_detail.id1)
            high_carb_lunch_2 = get_dish_by_id(high_carb_lunch_detail.id2)
            high_carb_dinner_1 = get_dish_by_id(high_carb_dinner_detail.id1)
            high_carb_dinner_2 = get_dish_by_id(high_carb_dinner_detail.id2)
        ...
        with col1:
            ...
            table_builder = '''
                <h3 style="text-align: center">Lower</h3>
                <table style="width: 100%;">
                    <tr>
                        <th>Exercise</th>
                        <th>Sets</th>
        ...
        with col2:
            ...
            table_builder = '''
                <h3 style="text-align: center">Upper</h3>
                <table style="width: 100%;">
                    <tr>
                        <th>Exercise</th>
                        <th>Sets</th>
