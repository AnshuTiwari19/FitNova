...
# Create database engine and fetch dish names
...
# Populate dish keywords for dropdown
dish_keywords = ['']
for d in all_dish_results:
    dish = Dish(*d)
    dish_keywords.append(dish.name)

# Center title and search dropdown
...
# Display dish details if a dish is selected
if dish_keyword != '':
    with engine.connect() as conn:
        dish_result = conn.execute(
            text("SELECT * FROM Dish WHERE Name = :name"),
            {'name': dish_keyword}
        ).fetchone()
        ...
        # Layout for image and nutrition details
        ...
        with col2:
            ...
            # Create pie chart for nutrition breakdown
